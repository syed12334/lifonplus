...
class Flashnews extends CI_Controller {   
    ...
    public function index(){
	  ...
	  public function category(){
        $this->load->view('flashnews/category_view',$this->data);
    }

    public function categoryList(){
        $det = $this->data['detail'];
        $data_list = $this->web_db->getFlashCategoryList($det);
        ...
        $_POST["length"] = -1;
        $members = $this->web_db->getFlashCategoryList($det);
    ...
    public function saveCategory(){
        ...
        if( !empty($_POST['name']) && !empty($_POST['id']) ){
            ...
            $check = $this->master_db->getRecords('flashnews_category',$where,'id');
            if( count($check) == 0 ){
                $update_data = array(
                    'name'          =>  $name,
                ...
                $this->master_db->updateRecord('flashnews_category',$update_data,array('id'=>$id));
                echo 1;
            }else{
            ...
            if( count($check) == 0 ){
                ...
                $id = $this->master_db->insertRecord('flashnews_category',$insert_data);
                echo 1;
            }else{
                echo "Category already exists.";
            }
    ...
    public function setCategoryStatus(){
        ...
        $this->master_db->updateRecord('flashnews_category',$where_data,array('id'=>$id));
        echo 1;
    }
    ...
    public function getCategory($id = ''){
        ...
        if( $id != '' ){
            ...
            $check = $this->master_db->getRecords('flashnews_category',$where,'id,name');
            if( count($check) == 0 ){
	...
	public function flashnews(){
        $this->load->view('flashnews/flashnews_view',$this->data);
    }

    public function flashnewsList(){
        $det = $this->data['detail'];
        $data_list = $this->web_db->getFlashnewsList($det);
        $log = $this->db->last_query();
        $file = 'sql.txt';
        //$json = "sdsd";
        $log = $log."\r\n";
        ...
        $_POST["length"] = -1;
        $flashnews = $this->web_db->getFlashnewsList($det);
   ...
   public function add_flashnews(){
		$this->data['type'] = 1;
		$this->data['category'] =  $this->master_db->getRecords('flashnews_category',array('status !='=>'-1'),'id,name');
        $this->data['customer'] =  $this->master_db->getRecords('customers',array('status ='=>'1'),'id,name');
        $this->load->view('flashnews/flashnews_add',$this->data);
    }
	
	
	public function edit_flashnews(){
		$this->data['type'] = 2;
		$id = trim($this->input->get('id'));
		$this->data['category'] =  $this->master_db->getRecords('flashnews_category',array('status !='=>'-1'),'id,name');
		$this->data['details'] =  $this->master_db->getRecords('flashnews',array('status !='=>'-1','id'=>$id),'id,title,cat_id,descr,image_path,date_time,notify,state_id,district_id,taluk_id');
    ...
    public function saveFlashnews(){
        ...
        if( !empty($_POST['cat_id']) && !empty($_POST['title']) && !empty($_POST['descr']) && !empty($_POST['id'])){
            ...
            $check = $this->master_db->getRecords('flashnews',$where,'id');
            if( count($check) == 0 ){
                $update_data = array(
                    'cat_id'      =>  $cat_id,
					'title'       =>  $title,
				...
				if( !empty($_FILES['image_path']['name']) ){
					$config = array();
					$config['upload_path'] = '../app_assets/flash_news/';  
					...
					if (!$this->upload->do_upload('image_path') && !empty($_FILES['image_path']['name'])) {
						...
						$update_data['image_path'] = 'flash_news/'.$upload_data['file_name'];
					}
				}
				
				
                $this->master_db->updateRecord('flashnews',$update_data,array('id'=>$id));
				$gcm_ids =  $igcm_ids  = array();
				 if(count($user_id))
				 {
					$this->master_db->updateRecord('flashnews_notify',array('status'=>'-1'),array('flash_id'=>$id));
					foreach($user_id as $u)
					{
						$check = $this->master_db->getRecords('flashnews_notify',array('flash_id'=>$id,'user_id'=>$u),'id');
						
						$insert_user = array('flash_id'=>$id,'user_id'=>$u,'status'=>1,'sent_at'=>date('Y-m-d H:i:s'));
						if(count($check) == 0)
						{
						   $this->master_db->insertRecord('flashnews_notify',$insert_user);
						}
						else{
							$this->master_db->updateRecord('flashnews_notify',array('status'=>1),array('flash_id'=>$id,'user_id'=>$u));
						}
						...
						if(@$custList[0]->device_type==2)
						{
						  $igcm_ids[] =@$custList[0]->device_id;
				                 ...
				                 $records = $this->master_db->getRecords('flashnews',array('id'=>$id),'image_path');
				...
				if($notify == 1 && count($gcm_ids)>0)
				{
					...
					{
						$message_status = $this->notify->sendNotification($regIdChunk[$k], $title,"Flashnews","https://www.lifeonplus.com/app_assets/".$records[0]->image_path);
				...
				if($notify==1 && count($igcm_ids)>0)
                {
                    ...
                    for($k=0;$k<count($regIdChunk);$k++)
                    {
                        $message_status = $this->notify->sendNotificationIOS($regIdChunk[$k], $title,"Flashnews");
                    }
                }
			    ...
			    redirect(base_url().'flashnews');
            }else{
                
				$this->session->set_flashdata('message','<div class="alert alert-danger alert-dismissable"><button class="close" aria-hidden="true" data-dismiss="alert" type="button">&times;</button> Flash News already exists.</div>');
			    redirect(base_url().'flashnews');
            }
        }
		else if( !empty($_POST['cat_id']) && !empty($_POST['title']) && !empty($_POST['descr'])){
            ...
            if( count($check) == 0 ){
                ...
                if (!$this->upload->do_upload('image_path') && !empty($_FILES['image_path']['name'])) {
                    ...
                    $upload_data = $this->upload->data();
                    //echo '<pre>';print_r($upload_data);exit;
                    $insert_data['image_path'] = 'flash_news/'.$upload_data['file_name'];
                }
            }
				
                $id = $this->master_db->insertRecord('flashnews',$insert_data);
				 ...
				 if(count($user_id))
				 {
					foreach($user_id as $u)
					{
						$insert_user = array('flash_id'=>$id,'user_id'=>$u,'status'=>1);
						$this->master_db->insertRecord('flashnews_notify',$insert_user);
    ...
    public function setFlashnewsStatus(){
        ...
        $this->master_db->updateRecord('flashnews',$where_data,array('id'=>$id));
        echo 1;
    }
