...
class Healthtips extends CI_Controller {   
	...
	public function healthtips(){
        $this->load->view('healthtips/healthtips_view',$this->data);
    }
	...
	public function add_healthtips(){
		$this->data['type'] = 1;
        // echo "<pre>";print_r( $this->data['detail']);exit;
		$this->data['category'] =  $this->master_db->getRecords('health_category',array('status !='=>'-1'),'id,name');
        $this->data['customer'] =  $this->master_db->getRecords('customers',array('status ='=>'1'),'id,name');
        $this->load->view('healthtips/healthtips_add',$this->data);
    }
	
	
	public function edit_healthtips(){
		$this->data['type'] = 2;
		$id = trim($this->input->get('id'));
		$this->data['category'] =  $this->master_db->getRecords('health_category',array('status !='=>'-1'),'id,name');
		$this->data['details'] =  $this->master_db->getRecords('healthtips',array('status !='=>'-1','id'=>$id),'id,title,cat_id,descr,image_path,date_time,notify,state_id,district_id,taluk_id');
        ...
        if( !empty($_POST['cat_id']) && !empty($_POST['title']) && !empty($_POST['descr']) && !empty($_POST['id'])){
            ...
            if( count($check) == 0 ){
				...
				if( !empty($_FILES['image_path']['name']) ){
					$config = array();
					$config['upload_path'] = '../app_assets/health_tips/';  
					...
					if (!$this->upload->do_upload('image_path') && !empty($_FILES['image_path']['name'])) {
						...
						$update_data['image_path'] = 'health_tips/'.$upload_data['file_name'];
					}
				}
               ...
               // echo "<pre>";print_r($imgfile[0]);exit;
                ...
                $this->master_db->updateRecord('healthtips',$update_data,array('id'=>$id));
				 ...
				 if(count($user_id))
				 {
					$this->master_db->updateRecord('health_notify',array('status'=>'-1'),array('health_id'=>$id));
					foreach($user_id as $u)
					{
						$check = $this->master_db->getRecords('health_notify',array('health_id'=>$id,'user_id'=>$u),'id');
						
						$insert_user = array('health_id'=>$id,'user_id'=>$u,'status'=>1);
						if(count($check) == 0)
						{
						   $this->master_db->insertRecord('health_notify',$insert_user);
						}
						else{
							$this->master_db->updateRecord('health_notify',array('status'=>1),array('health_id'=>$id,'user_id'=>$u));
						}
				...
				// echo "<pre>";print_r($imagefile);exit;
                $records = $this->master_db->getRecords('healthtips',array('id'=>$id),'image_path');
               // echo "<pre>";print_r($records);exit;
				...
				if($notify == 1 && count($gcm_ids)>0)
				{
					...
					{
						$message_status = $this->notify->sendNotification($regIdChunk[$k], $title,"Healthtips","https://www.lifeonplus.com/app_assets/".$records[0]->image_path);
				...
				if($notify==1 && count($igcm_ids)>0)
                {
                    ...
                    for($k=0;$k<count($regIdChunk);$k++)
                    {
                        $message_status = $this->notify->sendNotificationIOS($regIdChunk[$k], $title,"Healthtips");
                    }
                }
			    ...
			    redirect(base_url().'healthtips');
            }else{
                
				$this->session->set_flashdata('message','<div class="alert alert-danger alert-dismissable"><button class="close" aria-hidden="true" data-dismiss="alert" type="button">&times;</button> Health Tips already exists.</div>');
			    redirect(base_url().'healthtips');
            }
        }
            ...
            if( count($check) == 0 ){
                ...
                if (!$this->upload->do_upload('image_path') && !empty($_FILES['image_path']['name'])) {
                    ...
                    $upload_data = $this->upload->data();
                    //echo '<pre>';print_r($upload_data);exit;
                    $insert_data['image_path'] = 'health_tips/'.$upload_data['file_name'];
                }
            }
				// echo "<pre>";print_r($imgfile[0]);exit;
               ...
               // echo "<pre>";print_r($records);exit;
				...
				if($notify == 1 && count($gcm_ids)>0)
				{
					...
					for($k=0;$k<count($regIdChunk);$k++)
					{
						$message_status = $this->notify->sendNotification($regIdChunk[$k], $title,"Healthtips","https://www.lifeonplus.com/app_assets/health_tips/".$records[0]->image_path);
						//echo '________'.$message_status;exit;
					}
    ...
    public function setHealthtipsStatus(){
        $det = $this->data['detail'];
        $id = trim($this->input->post('id'));
        $status = trim($this->input->post('status'));
