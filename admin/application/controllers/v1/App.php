...
class App extends REST_Controller { 
        ...
        if( !empty($_POST['type']) && !empty($_POST['company_name']) && !empty($_POST['company_type']) 
            ...
            */
            ...
            if( $partner_id ){
                ...
                $this->data['type'] =$type;
                $mailbody = $this->load->view('end_users/end_user_mailview',$this->data,true);
    ...
    function homepage_post(){
        ...
        if( !empty($_POST['user_id']) ){   
            ...
            //echo $user_id;exit;
            $condition = " c.id = ".$user_id." and c.status = 1 ";
...
//echo '<pre>';print_r($checkCustomer);exit;
            //echo $checkCustomer[0]->qrcode;exit;
            ...
            if(count($checkCustomer)){
                if( $checkCustomer[0]->blood_group == '' ){
                    ...
                    if(empty($checkCustomer[0]->qrcode) ){
                        ...
                        $text = "https://www.lifeonplus.com/admin/v1/emergency_reports?user_id=".$checkCustomer[0]->user_id;
                        //echo $text;exit;
    ...
    function emergencyreport_get() {
       $result = array();
       //echo $_GET['user_id'];
        if(!empty($_GET['user_id'])){

                        $user_id = $this->input->get('user_id');
                        //$link = "https://www.lifeonplus.com/admin/v1/emergency_pdf?user_id=$user_id";
            ...
            $this->load->view('emergency_reports',$this->data);
                //echo $html;exit;
                ...
                //     $html2pdf->setDefaultFont('courier');
                //     $html2pdf->writeHTML($html);
                //     $pdf_name = $_GET['user_id'].'_'.time().'.pdf';
                //     $file_name = '../app_assets/card_image/'.$pdf_name;
                //     $file = $html2pdf->Output($file_name,'F');
            }else{
                $result = array('status'=>'failure','msg'=>'Customer not found.');
    ...
    function packages_post(){
        ...
        if( !empty($_POST['user_id']) ){
            $user_id = trim(preg_replace('!\s+!', '',html_escape($this->input->post('user_id', true))));
            // $customer = $this->master_db->sqlExecute('customers','id='.$user_id.' and status = 1','id');
            $customer = $this->master_db->sqlExecute('select cp.expiry_date,p.id as id,p.name as package_name,p.price as amount from customers c inner join customer_package cp on c.id=cp.customer_id inner join packages p on p.id=cp.package_id where c.id="'.$user_id.'" and c.status=1 and p.publish=1   order by cp.id desc limit 1 ');
            //echo $this->db->last_query();exit;
            if(count($customer)){
                $result = array('status'=>'success','data'=>$customer);     
            }else{
                $result = array('status'=>'failure','msg'=>'Customer not found.');
            }
        }        
    ...
    function packages_list_post(){
        ...
        if( !empty($_POST['user_id']) ){
            ...
            if(count($customer)){
                ...
                //$condition = "status = 1 and publish = 1";
    ...
    function view_flashnews_get(){
      ...
      function view_popupalerts_get(){
        $result = array('status'=>'failure','msg'=>'Required fields are missing.');
        if( !empty($_GET['user_id']) && !empty($_GET['pid']) ){
            $user_id = trim(preg_replace('!\s+!', '',html_escape($this->input->get('user_id', true))));
            $id = trim(preg_replace('!\s+!', '',html_escape($this->input->get('pid', true))));
            $customer =   $this->master_db->sqlExecute('select p.title,p.descr as description,p.image_path,p.created_at as datetime from popupalerts p left join popup_notify pn on pn.popup_id=p.id where pn.user_id = '.$user_id.' and p.id ='.$id.' order by pn.id desc limit 1');
            //echo $this->db->last_query();exit;
            if(count($customer)){
                $this->data['popup'] =$customer;
                $this->load->view('popupalerts_view',$this->data);
            }else{
                echo 'Customer not found.';
            }
        }else{
    ...
    function view_notification_get(){
        ...
        if( !empty($_GET['user_id']) ){
            ...
            if(count($customer)){
                $condition = "rn.user_id= $user_id and rn.status = 1 and n.status = 1";
                $this->data['notify'] = $this->app_db->getnotifications($condition,0,10);
        ...
        if( !empty($_POST['user_id']) && !empty($_POST['type'])  ){
            ...
            if(count($customer)){   
                ...
                if( ( !empty($_POST['latitude']) && !empty($_POST['longitude']) ) ){
                    ...
                    $location = "https://maps.google.com/maps?daddr=".$latitude.",".$longitude;
                }
    ...
    function coupons_list_post(){
        ...
        if( !empty($_POST['user_id']) ){
            ...
            if(count($customer)){
                $today = date('Y-m-d');
                $condition = " ('".$today."'  between from_date and to_date) and status = 1 and  find_in_set(".$user_id.",cname)";
        ...
        if( !empty($_POST['user_id']) ){
            ...
            if(count($customer)){
                ...
                $list = $this->app_db->getpackagemodules($condition);
                    ...
                    $result = array('status'=>'failure','msg'=>'Error...');
                }
...
function package_upgrade_post(){
        $result = array('status'=>'failure','msg'=>'Required fields are missing.');
        if( !empty($_POST['user_id']) && !empty($_POST['package_id']) && !empty($_POST['package_amont']) && !empty($_POST['total_amount']) && isset($_POST['coupon_amount']) && isset($_POST['razor_pay_amount']) && !empty($_POST['type']) && isset($_POST['coupon_id']) && isset($_POST['expiry_date']) ){
            //echo '<pre>';print_r($_POST);exit;
            $user_id = trim(preg_replace('!\s+!', '',html_escape($this->input->post('user_id', true))));
            //echo $user_id;
            $package_id = trim(preg_replace('!\s+!', '',html_escape($this->input->post('package_id', true))));
            ...
            if(count($customer)){
                ...
                $checkpackage = $this->master_db->getRecords('packages',$condition,'id,price,expiry_date,validity');
                //echo "<pre>";print_r($checkpackage);exit;
                ...
                if(count($checkcp)){
                    if( $type == 'upgrade' ){
                         $validity = $checkpackage[0]->validity;
                            $expiry = date('Y-m-d' ,strtotime("+$validity days"));
                         $this->master_db->updateRecord('customer_package',array('expiry_date'=>$expiry,'pstatus'=>2),array('id'=>$checkcp[0]->id));
                        $where = ['customer_id'=>$user_id,'package_id'=>$package_id];
                        $customer_package = $this->master_db->getRecords('customer_package',$where);
                        //echo $this->db->last_query();exit;
                        if(count($customer_package) >0) {
                             $validity = $checkpackage[0]->validity;
                            $expiry = date('Y-m-d' ,strtotime("+$validity days"));
                            $update = array('expiry_date'=>$expiry,'updated_at'=>date('Y-m-d H:i:s'));
                            $this->master_db->updateRecord('customer_package',$update,array('customer_id'=>$user_id,'package_id'=>$package_id));
   ...
   function popupalerts_post(){
        //echo '<pre>';print_r($_GET);exit;
        $result = array('status'=>'failure','msg'=>'Required fields are missing.');
        if( !empty($_POST['user_id']) ){ 
            $user_id = trim(preg_replace('!\s+!', '',html_escape($this->input->post('user_id', true))));
            $customer = $this->master_db->sqlExecute('select p.id as pid,p.title,p.descr as description,p.image_path,p.created_at as date from popupalerts p left join popup_notify pn on pn.popup_id=p.id where pn.user_id = '.$user_id.'  order by pn.id desc limit 1');
            //echo $this->db->last_query();exit;
            if(count($customer) >0) {
                $customer[0]->image_path = app_asset_url().$customer[0]->image_path;
                $customer[0]->date = date('d-m-Y',strtotime($customer[0]->date));
                $result = array('status'=>'success','msg'=>$customer);
            }else {
                $result = array('status'=>'failure','msg'=>'Customer not found.');
            }            
        }
       ...
       function tests_post(){
        ...
        $result = ['status'=>'failure','msg'=>'Required fields is missing'];
         $diagnostic_id = trim(preg_replace('!\s+!', '',html_escape($this->input->post('diagnostic_id', true))));
         if(!empty($diagnostic_id)) {
            $tests = $this->master_db->getRecords('test_modules',['status !='=>-1,'did'=>$diagnostic_id],'id,test_name,mrp,price,report_time,special_note,created_at as created_date');
            //echo $this->db->last_query();exit;
            if(count($tests) >0) {
                foreach ($tests as $key => $value) {
                    $id = $value->id;
                    $value->created_date = date('d-m-Y',strtotime($value->created_date));
                    $test_includes = $this->master_db->getRecords('test_includes',['tid'=>$id],'id,title');
                    $value->includes = $test_includes;
                }
                 $result =['status'=>'success','msg'=>$tests];
            }else {
     ...
     function diagnostic_get(){
        //echo '<pre>';print_r($_GET);exit;
        $result = [];
            $diagnostic = $this->master_db->getRecords('diagnostic_category',['status'=>1],'id,name,created_at as added_on');
            //echo $this->db->last_query();exit;
            if(count($diagnostic) >0) {
                foreach ($diagnostic as $key => $value) {
                    $value->added_on= date('d-m-Y',strtotime($value->added_on));
                }
                 $result =['status'=>'success','msg'=>$diagnostic];
            }else {
                $result = ['status'=>'failure','msg'=>[]];
            }
        echo json_encode($result);
    }
    function checkups_post(){
        //echo '<pre>';print_r($_GET);exit;
        $result = ['status'=>'failure','msg'=>'Required fields is missing'];
        $diagnostic_id = trim(preg_replace('!\s+!', '',html_escape($this->input->post('diagnostic_id', true))));
            if(!empty($diagnostic_id)) {
                $tests = $this->master_db->getRecords('checkup_modules',['status !='=>-1,'did'=>$diagnostic_id],'id,test_name as checkup_name,mrp,price,report_datetime as report_time,special_note,created_at as created_date');
                if(count($tests) >0) {
                    foreach ($tests as $key => $value) {
                        $id = $value->id;
                        $value->created_date = date('d-m-Y',strtotime($value->created_date));
                        $test_includes = $this->master_db->getRecords('checkup_includes',['cid'=>$id],'id,title');
                        $value->includes = $test_includes;
                    }
                     $result =['status'=>'success','msg'=>$tests];
                }else {
      ...
      function timeslot_post(){
        //echo '<pre>';print_r($_GET);exit;
            $result = [];
            $day_no = trim(preg_replace('!\s+!', '',html_escape($this->input->post('day_no', true))));
            $where ="";
            if(!empty($day_no)) {
                $where = " and days=".$day_no;
            }   
            $timeslotm = $this->master_db->sqlExecute('select id,days,wday,timings,created_at as created_date from timeslot where status !=-1 and wday =1 '.$where.'');
            $timeslota = $this->master_db->sqlExecute('select id,days,wday,timings,created_at as created_date from timeslot where status !=-1 and wday=2 '.$where.'');
            $timeslote = $this->master_db->sqlExecute('select id,days,wday,timings,created_at as created_date from timeslot where status !=-1 and wday=3 '.$where.'');
            //echo $this->db->last_query();exit;
            if(count($timeslotm)) {
                foreach ($timeslotm as $tm) {
                    $tm->created_date = date('d-m-Y',strtotime($tm->created_date));
                    $day ="";$d = "";
                    if($tm->days ==1) {
                        $day ="Monday";
                    }else if($tm->days ==2) {
        ...
        $result =['status'=>'success','morning'=>$timeslotm,'afternoon'=>$timeslota,'evening'=>$timeslote];
        echo json_encode($result);
    }
      public function booktests_post() {
            $result = array('status'=>'failure','msg'=>'Required fields are missing.');
            ...
            $name = trim(preg_replace('!\s+!', '',$confirmOrders['name']));
            $mobile = trim(preg_replace('!\s+!', '',$confirmOrders['mobile']));
            $gender = trim(preg_replace('!\s+!', '',$confirmOrders['gender']));
            $age = trim(preg_replace('!\s+!', '',$confirmOrders['age']));
            $total_amount = trim(preg_replace('!\s+!', '',$confirmOrders['total_amount']));
            $sub_total = trim(preg_replace('!\s+!', '',$confirmOrders['sub_total']));
            $coupon_id = trim(preg_replace('!\s+!', '',$confirmOrders['coupon_id']));
            $coupon_amount = trim(preg_replace('!\s+!', '',$confirmOrders['coupon_amount']));
            $time_slot = $confirmOrders['time_slot'];
            $address = $confirmOrders['address'];
            $checkup = $confirmOrders['checkup'];
            $tests = $confirmOrders['tests'];
            $paymentapi = TEST_MERCHANT_KEY;
            if(!empty($user_id) && !empty($name) && !empty($mobile) && !empty($gender) && !empty($age) && !empty($total_amount) && !empty($sub_total) && is_array($address) && is_array($time_slot) || is_array($checkup) || is_array($tests)) {
            //echo json_encode($time_slot);
            $user_exists = $this->master_db->getRecords('customers',['id'=>$user_id],'*');
            if(count($user_exists)) {
                ...
                $db['sub_total'] = $sub_total;
                $db['coupon_id'] = $coupon_id;
                $db['coupon_amount'] = $coupon_amount;
                $db['address'] = json_encode($address);
                $db['time_slot'] = json_encode($time_slot);
                $db['tests'] = json_encode($tests);
                $db['checkups'] = json_encode($checkup);
                $db['paymode'] = 'Online';
                $db['status'] = 0;
                $db['created_at'] = date('Y-m-d H:i:s');
                $in = $this->master_db->insertRecord('orders',$db);
                if(!empty($in)) {
                    $orderNo = $this->app_db->generateOrderNo($in);
                    $dbs = array('orderno' => $orderNo);
                    $this->master_db->updateRecord('orders',$dbs,['id'=>$in]);
                }
                $api = new Api(TEST_MERCHANT_KEY, TEST_MERCHANT_SECRET);
                $razor_amt = $total_amount *100;
                $currency = "INR";
                $order  = $api->order->create(array('receipt' =>rand(11111,99999), 'amount' => $razor_amt, 'currency' => $currency));
                $orderId = $order['id'];
                $p['oid'] = $in;
                $p['order_id'] = $orderId;
                $p['paid_amount'] = $total_amount;
                $p['status'] = -1;
                $p['created_at'] =date('Y-m-d H:i:s');
                $this->master_db->insertRecord('payment_log',$p);
                $getrecords =$this->master_db->sqlExecute('select * from orders o left join payment_log p on p.oid =o.id where o.id ='.$in.'');
                $paid_amount['paid_amount'] = $getrecords[0]->paid_amount;
                $paid_amount['payment_mode'] = $getrecords[0]->paymode;
                $paid_amount['orderno'] = $getrecords[0]->orderno;
                $paid_amount['razorpay_orderid'] = $getrecords[0]->order_id;
                $paid_amount['razorpay_apikey'] = $paymentapi;
    ...
    public function razorpayResponse_post() {
            ...
            if(!empty($user_id) && !empty($paymentID) && !empty($orderID) && !empty($signature) && !empty($oid)) {
                ...
                $getRecords = $this->master_db->getRecords('orders',['orderno'=>$oid],'id');
                 ...
                 if($generatedHash ==$signature) {
                    ...
                    $this->master_db->updateRecord('orders',$dbp,['id'=>$getRecords[0]->id]);
                     $payarr = array(
                            "payment_id"=>$paymentID,
                            "signature"=>$signature,
                     );
                     $this->master_db->updateRecord('payment_log',$payarr,['oid'=>$getRecords[0]->id]);
        ...
        public function fetchOrders_post() {
            $result = array('status'=>'failure','msg'=>'Required fields are missing.');
            if(!empty($_POST['user_id'])) {
                $user_id = trim(preg_replace('!\s+!', '',html_escape($this->input->post('user_id', true))));
                $orders = $this->master_db->getRecords('orders',['user_id'=>$user_id],'id as oid,orderno as order_no,time_slot,status','id desc');
                if(count($orders)) {
                    foreach ($orders as $key => $value) {
                        $time = $value->time_slot;
                        $decode = json_decode($time,true);
                        $value->time_slot = $decode;
                    }
        ...
        public function orderDetails_post() {
            ...
            if(!empty($_POST['order_id'])) {
                $order_id = trim(preg_replace('!\s+!', '',html_escape($this->input->post('order_id', true))));
                $orders = $this->master_db->getRecords('orders',['id'=>$order_id],'id as oid,orderno as order_no,name,mobile,gender,age,total_amount,coupon_id,time_slot,tests,checkups,time_slot,address,status,created_at');
                if(count($orders)) {
                    foreach ($orders as $key => $value) {
                        $time = $value->time_slot;
                        $timed = json_decode($time);
                        $value->time_slot = $timed;
                        $value->created_at = date('d-m-Y',strtotime($value->created_at));
                        $address = $value->address;
                        $addressd = json_decode($address);
                        $value->address = $addressd;

                        $tests = $value->tests;
                        $testsd = json_decode($tests,true);
                        $value->tests = $testsd;

                        $checkups = $value->checkups;
                        $checkupsd = json_decode($checkups,true);
                        $value->checkups = $checkupsd;
                    }
                    $result = ['status'=>'success','order_details'=>$orders];
                }else {
                    $result = ['status'=>'failure','msg'=>"No data found"];
                }
            }
        ...
        public function country_get() {
           $country = $this->master_db->getRecords('countries',['status!='=>0],'id,name');
           $result =[];
           if(count($country)) {
                $result = ['status'=>'success','country'=>$country];
           }else {
                $result = ['status'=>'failure','msg'=>'No data found'];
           }
           echo json_encode($result);
        ...
        public function state_post() {
            ...
            if(!empty($_POST['country_id'])) {
                $country_id = trim(preg_replace('!\s+!', '',html_escape($this->input->post('country_id', true))));
                $state = $this->master_db->getRecords('states',['country_id'=>$country_id],'id,name');
               if(count($state)) {
                    $result = ['status'=>'success','states'=>$state];
               }else {
                    $result = ['status'=>'failure','msg'=>'No data found'];
               }
            }
        ...
        public function district_post() {
            ...
            if(!empty($_POST['state_id'])) {
                $state_id = trim(preg_replace('!\s+!', '',html_escape($this->input->post('state_id', true))));
                $district = $this->master_db->getRecords('districts',['state_id'=>$state_id],'id,name');
               if(count($district)) {
                    $result = ['status'=>'success','district'=>$district];
               }else {
                    $result = ['status'=>'failure','msg'=>'No data found'];
               }
            }
        ...
        public function city_post() {
            ...
            if(!empty($_POST['district_id'])) {
                ...
                $city = $this->master_db->getRecords('cities',['district_id'=>$district_id],'id,name');
               if(count($city)) {
        ...
        public function videoapi_post() {
            $result = [];
            if(!empty($_POST['user_id'])) {
                $user_id = trim(preg_replace('!\s+!', '',html_escape($this->input->post('user_id', true))));
                $getCustomers = $this->master_db->getRecords('customers',['id'=>$user_id],'name,email,mobileno,code');
                if(count($getCustomers) >0) {
                   $name = $getCustomers[0]->name;
                    $email = $getCustomers[0]->email;
                    $mobileno = $getCustomers[0]->mobileno;
                    $code = $getCustomers[0]->code;
                    $appID =APP_ID;
                    $appCertificate =APP_CERTIFICATE;
                    $channelName =trim($getCustomers[0]->code);
                    $uid =$user_id;
                    $usid = rand(1234,5678);
                    $role = 0;
                    ...
                    $token = $this->agora->buildTokenWithUid($appID, $appCertificate, $channelName, $uid, $role, $privilegeExpiredTs);
                    $tokenweb = $this->agora->buildTokenWithUid($appID, $appCertificate, $channelName,$usid, $role, $privilegeExpiredTs);
                    $v['name'] = $name;
                    $v['email'] = $email;
                    $v['mobile'] = $mobileno;
                    $v['appid'] = $appID;
                    $v['apptoken'] = trim($token);
                    $v['channelname'] = $channelName;
                    $v['role'] = $role;
                    $v['user_id'] = $user_id;
                    $v['uid'] = $uid;
                    $v['webtoken'] = trim($tokenweb);
                    $v['wuid'] =$usid;
                    $v['created_at'] = date('Y-m-d H:i:s');
                    $v['status'] = 0;
                    $curdate = strtotime(date('Y-m-d H:i:s'));
                    $getVideo = $this->master_db->getRecords('videoapi',['user_id'=>$user_id],'*');
                    if(count($getVideo) >0) {
                        $tokena = @strtotime($getVideo[0]->created_at);
                        $new = $curdate-$tokena;
                        if($new >86400) {
                            $up = $this->master_db->updateRecord('videoapi',['apptoken'=>$token,'channelname'=>$channelName,'webtoken'=>$tokenweb,'wuid'=>$usid,'created_at'=>date('Y-m-d H:i:s')],['user_id'=>$user_id]);
                             $resp = ['appid'=>APP_ID,'token'=>$token,'channel_name'=>$getVideo[0]->channelname];
                            $result = array('status'=>'success','video'=>$resp);
                        }else {
                            $tokenf = $getVideo[0]->apptoken;
                            $cname = $getVideo[0]->channelname;
                            $resp = ['appid'=>APP_ID,'token'=>$tokenf,'channel_name'=>$getVideo[0]->channelname];
                            $result = array('status'=>'success','video'=>$resp);
                        }
                    }else {
                        $in = $this->master_db->insertRecord('videoapi',$v);
                        if($in) {
                            $resp = ['appid'=>APP_ID,'token'=>$token,'channel_name'=>$getCustomers[0]->code];
                            $result = array('status'=>'success','video'=>$resp);
                        } 
                    }
                }else {
                ...
                $result = array('status'=>'failure','msg'=>'Userid is missing.');
            }
            echo json_encode($result);
        }
        public function vitalReports_post() {
            $result = array('status'=>'failure','msg'=>'Required fields are missing.');
            //echo "<pre>";print_r($_POST);exit;
            if(!empty($_POST['user_id'])) {
                $user_id = trim(preg_replace('!\s+!', '',html_escape($this->input->post('user_id', true))));
                $height = trim(preg_replace('!\s+!', '',html_escape($this->input->post('height', true))));
                $weight = trim(preg_replace('!\s+!', '',html_escape($this->input->post('weight', true))));
                $bmi = trim(preg_replace('!\s+!', '',html_escape($this->input->post('bmi', true))));
                $mac = trim(preg_replace('!\s+!', '',html_escape($this->input->post('mac', true))));
                $waist = trim(preg_replace('!\s+!', '',html_escape($this->input->post('waist', true))));
                $hip = trim(preg_replace('!\s+!', '',html_escape($this->input->post('hip', true))));
                $temp = trim(preg_replace('!\s+!', '',html_escape($this->input->post('temp', true))));
                $sys = trim(preg_replace('!\s+!', '',html_escape($this->input->post('sys', true))));
                $dia = trim(preg_replace('!\s+!', '',html_escape($this->input->post('dia', true))));
                $pulse = trim(preg_replace('!\s+!', '',html_escape($this->input->post('pulse', true))));
                $spo2 = trim(preg_replace('!\s+!', '',html_escape($this->input->post('spo2', true))));
                $ecg = html_escape($this->input->post('ecg'));
                $blood_sugar_type = html_escape($this->input->post('blood_sugar_type'));
                $hb = trim(preg_replace('!\s+!', '',html_escape($this->input->post('hb', true))));
                $glucose = trim(preg_replace('!\s+!', '',html_escape($this->input->post('glucose', true))));
                $vascular_level = trim(preg_replace('!\s+!', '',html_escape($this->input->post('vascular_level', true))));
                $skin_carotonoid = trim(preg_replace('!\s+!', '',html_escape($this->input->post('skin_carotonoid', true))));
                $astigmatism = trim(preg_replace('!\s+!', '',html_escape($this->input->post('astigmatism', true))));
                $visual_acuity = trim(preg_replace('!\s+!', '',html_escape($this->input->post('visual_acuity', true))));
                $duochrome = trim(preg_replace('!\s+!', '',html_escape($this->input->post('duochrome', true))));
                $color_test = trim(preg_replace('!\s+!', '',html_escape($this->input->post('color_test', true))));
                $getCustomers = $this->master_db->getRecords('customers',['id'=>$user_id],'*');
                if(count($getCustomers) >0) {
                    $db['user_id'] = $user_id;
                    $db['height'] = $height;
                    $db['weight'] = $weight;
                    ...
                    $db['spo2'] = $spo2;
                    $db['ecg'] = $ecg;
                    $db['blood_sugar_type'] = $blood_sugar_type;
                    $db['hb'] = $hb;
                    $db['glucose'] = $glucose;
                    $db['vascular_level'] = $vascular_level;
                    $db['skin_carotonoid'] = $skin_carotonoid;
                    $db['visual_acuity'] = $visual_acuity;
                    $db['astigmatism'] = $astigmatism;
                    $db['duochrome'] = $duochrome;
                    $db['color_test'] = $color_test;
                    $db['created_at'] = date('Y-m-d H:i:s');
                    $in = $this->master_db->insertRecord('vital_reports',$db);
                    if($in) {
                        $result = ['status'=>'success','report_id'=>$in];
                    }else {
                        $result = ['status'=>'failure','msg'=>'error in inserting'];
                    }
                }else {
                    $result = ['status'=>'failure','msg'=>'Customer not found'];
                }
            }
            echo json_encode($result);
        }
        public function viewReports_post() {
            $result = array('status'=>'failure','msg'=>'Required fields are missing.');
            if(!empty($_POST['user_id'])) {
                $user_id = trim(preg_replace('!\s+!', '',html_escape($this->input->post('user_id', true))));
                $getVitals = $this->master_db->getRecords('vital_reports',['user_id'=>$user_id],'id as report_id,height,weight,bmi,mac,waist,hip,temp,sys,dia,pulse,spo2,ecg,blood_sugar_type,hb,glucose,created_at as report_date,vascular_level, skin_carotonoid, astigmatism, visual_acuity, duochrome, color_test','id desc');
                if(count($getVitals) >0) {
                    foreach ($getVitals as $key => $value) {

                        $value->report_date = date('d-m-Y H:i:s',strtotime($value->report_date));
                    }
                    $result = ['status'=>'success','reports'=>$getVitals];
                }else {
                    $result = ['status'=>'success','msg'=>'UserID not found'];
                }
            }
            echo json_encode($result);
        }
        public function mobtracking_get() {
            $getData = $this->master_db->getRecords('tracking',['id !='=>-1],'*');
            if(count($getData) >0) {
                $result = ['status'=>'success','reports'=>$getVitals];
            }else {
                $result = ['status'=>'failure','msg'=>'No data found'];
        ...
        public function packagewithpayment_post() {
            $result = array('status'=>'failure','msg'=>'Required fields are missing.');
            $confirm = file_get_contents('php://input');
            $confirmOrders = json_decode($confirm, true);
            $user_id = trim(preg_replace('!\s+!', '',$confirmOrders['user_id']));
            $package_id = trim(preg_replace('!\s+!', '',$confirmOrders['package_id']));
            $package_amount = trim(preg_replace('!\s+!', '',$confirmOrders['package_amont']));
            $coupon_amount = trim(preg_replace('!\s+!', '',$confirmOrders['coupon_amount']));
            $total_amount = trim(preg_replace('!\s+!', '',$confirmOrders['total_amount']));
            $razor_pay_amount = trim(preg_replace('!\s+!', '',$confirmOrders['razor_pay_amount']));
            $coupon_id = trim(preg_replace('!\s+!', '',$confirmOrders['coupon_id']));
            $expiry_date = trim(preg_replace('!\s+!', '',$confirmOrders['expiry_date']));
            $type = trim(preg_replace('!\s+!', '',$confirmOrders['type']));
            $paymentapi = TEST_MERCHANT_KEY;
            //echo "<pre>";print_r($confirmOrders);exit;
        if(!empty($user_id) && !empty($package_id) && !empty($package_amount) && !empty($total_amount) && isset($coupon_amount) && isset($razor_pay_amount) && !empty($type) && isset($coupon_id) && isset($expiry_date) ){
             $customer = $this->master_db->getRecords('customers','id='.$user_id.' and status = 1','id,code,name,mobileno,gender,address');
             if(count($customer) >0){
                 $checkcoupon = array();
                if( !empty($coupon_id) ){
                    $condition = 'id='.$coupon_id.' and status = 1 and no_of_users > uses ';
                ...
                $checkcp = $this->master_db->getRecords('packages',['id'=>$package_id,'status'=>1],'*');
                if(count($checkcp) >0) {
                     if($type =='upgrade'){
                         $validity = $checkpackage[0]->validity;
                            $expiry = date('Y-m-d' ,strtotime("+$validity days"));
                        ...
                        $orders['user_id'] = $user_id;
                            $orders['name'] = $customer[0]->name;
                            $orders['mobile'] = $customer[0]->mobileno;
                            if($customer[0]->gender ==1) {
                                $orders['gender'] = "Male";
                            }
                            else if($customer[0]->gender ==0) {
                                $orders['gender'] = "Female";
                            }
                            else if($customer[0]->gender ==2) {
                                $orders['gender'] = "Transgender";
                            }
                            $orders['total_amount'] = $razor_pay_amount;
                            $orders['coupon_id'] = $coupon_id;
                            $orders['coupon_amount'] = $coupon_amount;
                            $orders['address'] = json_encode(['address'=>$customer[0]->address]);
                            $orders['paymode'] = 'Online';
                            $orders['status'] = 0;
                            $orders['created_at'] = date('Y-m-d H:i:s');
                            $ins = $this->master_db->insertRecord('orders',$orders);
                            if(!empty($ins)) {
                                $orderNo = $this->app_db->generateOrderNo($ins);
                                $dbs = array('orderno' => $orderNo);
                                $this->master_db->updateRecord('orders',$dbs,['id'=>$ins]);
                            }
                            $api = new Api(TEST_MERCHANT_KEY, TEST_MERCHANT_SECRET);
                            $razor_amt = $razor_pay_amount *100;
                            $currency = "INR";
                            $order  = $api->order->create(array('receipt' =>rand(11111,99999),'amount' => $razor_amt, 'currency' => $currency));
                            $orderId = $order['id'];
                            if(!empty($ins)) {
                                $ps['oid'] = $ins;
                                $ps['order_id'] = $orderId;
                                $ps['paid_amount'] = $total_amount;
                                $ps['status'] = -1;
                                $ps['created_at'] =date('Y-m-d H:i:s');
                                $this->master_db->insertRecord('payment_log',$ps);
                            }
                            $getrecords =$this->master_db->sqlExecute('select * from orders o left join payment_log p on p.oid =o.id where o.id ='.$ins.'');
                            $paid_amount['paid_amount'] = $getrecords[0]->paid_amount;
                            $paid_amount['payment_mode'] = $getrecords[0]->paymode;
                            $paid_amount['orderno'] = $getrecords[0]->orderno;
                            $paid_amount['razorpay_orderid'] = $getrecords[0]->order_id;
                            ...
                            $paid_amount['currency_type'] = 'INR';
                            $paid_amount['razor_callback'] = 'razorpayResponse';
                            $result = array('status'=>'success','payment_response'=>$paid_amount);
                    }
                    else if($type == 'renew') {
                         $validity = $checkpackage[0]->validity;
                            $expiry = date('Y-m-d' ,strtotime("+$validity days"));
                      ...
                      $this->master_db->updateRecord('customer_package',array('expiry_date'=>$expiry,'pstatus'=>2),array('id'=>$checkcp[0]->id));  
                        ...
                        $result = array('status'=>'failure','msg'=>'No type is found.');
                    }
                }else {
                    $result = array('status'=>'failure','msg'=>'No package is found.');
                }
