...
class Home_db extends CI_Model{
	...
	function getencryptPass($pass, $encrypt=1){
	    require_once '/var/www/html/admin/includes/PkcsKeyGenerator.php';
	    require_once '/var/www/html/admin/includes/DesEncryptor.php';
	    require_once '/var/www/html/admin/includes/PbeWithMd5AndDes.php';
	...
	function getlogin($db=array(), $check){
	    ...
	    $wdb = array("l.status != "=> -1, "l.phone"=>$db['phone']);
	    $pass = "";
	    if($check){
	        $wdb["l.password"] = $this->getencryptPass($db['password']);
        }	    
        $this->db->select("l.id,p.id as pid, l.phone, l.password, l.status, l.partner_id, p.type, pp.photo,pp.country_id,pp.state_id,pp.district_id,pp.taluk_id")
				 ->from('partner_login l')
				 ->join('partners p','p.id=l.partner_id')
