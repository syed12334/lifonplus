    ...
    <body>
        ...
        <script>
            ...
            $(document).ready(function(){ 
                ...
                $("#saveButton").click(function(evt){
                    ...
                    if(v.form()){
                        ...
                        $.post("<?=base_url().'master/saveTests'?>", str, function(data){
                            if(parseInt(data) == 1){
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            ...
                            url: "<?=base_url().'master/setTestsStatus'?>",
                            type: "post",
                            data:  postdata ,
                            dataType : 'json',
            ...
            function modifyRow(id){
                console.log(id)
                $('#addModal').modal('show')
                $('#addModalLabel').html('Edit Tests')

                $.get("<?=base_url().'master/getTests/'?>"+id, {id:id}, function(data){
                    try{
                       ...
                       // console.log(json);
                        if( json.status == 'success' ){
                            ...
                            $('#time').val(json.data.report_time);
                            $('#id').val(json.data.id);
							var row='';
							...
							if((json.subdata).length !=0)
							{
								...
								//$.each(json.subdata[i], function (key, val) {
									...
									row +="<td><input type='hidden'  name='subid[]' value="+json.subdata[i].id+"><input type='text'  name='name[]' id='name' class='form-control' value='"+json.subdata[i].title+"'></td>";
									if(i==0)
									{
									   row +="<td><a class='btn btn-sm btn-primary' onclick='addrow();' style='cursor: pointer;'><i class='fa fa-plus'></i></a></td>";
									}
		...
		function removedata(id)
		{
			...
			{	
                 $("#trs-"+id).remove();
            $.ajax({
                url :"<?= base_url().'master/deleteTests'; ?>",
                method:"post",
                dataType:"json",
                data :{
                    id:id
                        ...
                        <div class="modal-body">
                            <form id="category_form">
							    ...
							    <div class="form-group">
                                    ...
                                    <input type="text" name="test_name" id="test_name" placeholder="Test Name" class="form-control " required title="Enter Test Name"/>
